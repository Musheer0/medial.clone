...
function MobileNav(){
  ...
  return <div className='flex  z-[999999] i lg:hidden border-t border-zinc-900 fixed bottom-0 left-0 item-center gap-4 bg-zinc-950  w-full justify-between py-1 px-2'>
    {mobileLinks.map((e)=>{
            if(pathname===e.path)      return <Link key={e.path} className='flex flex-col   items-center leading-none justify-between text-center border-t-2 border-purple-400 text-purple-400 px-4 py-3 ' href={e.path}>{e.icon} <span className='text-xs'>{e.name}</span></Link>
